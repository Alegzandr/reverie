...
- **Waveform (`WaveformTimeline`)**: the centre instrument - a `.hud-frame` glass plate holding the loaded track's mirrored envelope, rendered holographically. Every bar is centred on a shared instrument axis (a faint `--hud-line` hairline spanning the clip), and its vertical gradient puts a hot core on that centreline - together the bars draw one continuous luminous spine through the waveform. Played bars burn in the mood accent around the `--wf-core` hot core (near-white light on dark moods, a deep saturated ink on the light workspace, where white-on-white would vanish); unplayed bars wait in the cool `--hud-line` hue, so progress reads as temperature, not just brightness. A soft accent wash pools along the played region, brightest at the playhead, and the playhead itself is a light blade with a round reading head where it crosses the axis. A header reads status + clock (`Now playing` / `0:00 / total`), a footer reads the live effect (`STANDBY` / `0.70× · 50% RV`), and the envelope reshapes live to preview the active effect. Click, drag, or arrow-key to seek (`role="slider"`); the bars are non-interactive so they never intercept a seek. Its aura (`.wf-aura`) lifts with level + treble.
