...
- **Music** (`audioFeed.ts`): a tee analyser off the playback graph (`analyserSource.ts`) folds 64 log bands into a scrolling spectrum-history texture (60 rows/s), plus level/bass/mid/treble and kick detection. Each world spends it in its own way, and always in its **light, never its shape** (land, water and rock never move with the music): swells of moonlight rolling down the glade and the lighthouse lamp burning with the level, a breath and a gliding band of light along the nebula's lace, the corona carrying the spectrum outward, the fjord's lake grid replaying it toward the sun with kicks climbing the wireframes, warmer rims on daybreak's clouds - contemplative, never a visualiser. Gaussian falloffs use the prelude's `sq()`, never `pow(x, 2.0)` (undefined for a negative base: NaN, which the TAA spreads as black), and no unbounded `exp()` may feed a `mix()` (inf × 0 is NaN too).
