...
Reverie is a free, fully in-browser audio editor that turns any track into an atmospheric remix. Four effects: Speed Up, Slowed + Reverb, 8D Audio, Bass Boost, heard live over a playlist the app remembers on the device (files included), so a whole evening of music is one drop away. All processing runs client-side through the Web Audio API, so files never leave the device (privacy by architecture). Exports mirror the source format and quality. The app ships in 10 languages and installs as a PWA. It is a deliberately **desktop-only** experience: the cockpit (raked effects and playlist consoles around a living world) needs a wide canvas to feel like itself, so viewports narrower than 1024px are gated to a branded "open on a larger screen" stage with no bypass (browser zoom on a desktop excepted: someone zooming in to read keeps the app) - we would rather send a phone visitor to a computer than ship a cramped layout. Success looks like a first-time visitor uploading a track and downloading a transformed version they are proud to share, without ever needing a tutorial.
...
4. **Atmosphere over chrome.** The dreamy mood is a feature, not decoration. You listen as if through a spacesuit visor: the interface is a holographic HUD projected on the glass, and beyond it a real-time living world (a nebula, an aurora over a mirror lake, a moonlit sea...) that listens to the music. Contemplative, never a visualiser. Listeners pick a mood to change the whole world in one tap. Motion, color, and the waveform should make transforming audio feel immersive - and all of it has a calm, static fallback under reduced motion.
